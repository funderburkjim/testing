package getfiles;
use Exporter ();
# work/stcscan/getfiles.pm
@ISA = qw(Exporter);
@EXPORT = qw(getfiles);
# expect parameter to be a filename with directory
# e.g. /docs/scans/STCScan/STCScanjpg/xyz.jpg
...
#    print "dir = $dir, match = $match, sfx = $sfx\n";
    my $filename="../../../docs/scans/STCScan/stcfiles.txt";
    my %MYFILE;
...
#	print "DBG: file not found\n";
	...
	chomp($line);
	$line =~ s/\r//; # in case line has a carriage return at end
	if ($line eq $match) {
	    $next=<MYFILE>;
	    chomp($next);
	    $more = 0;

...
if (!$keyin and $ARGV[0]) {$keyin = $ARGV[0];}
...
print STDERR "chk: keyin = $keyin, key = $key\n";
my $err = 0;
...
#    $sql = "select * from `$table` where `key`=\"$key\"";
	...
	print STDERR "chk1: data1=$data1, page=$page, key=$key, id=$id\n";
	$ans .= $temp;
	$n++;
	}else {
	    print STDERR " skipping id = $id, n=$n\n";
	}
    }
    if ($n != 0) {
	...
	my ($left,$top,$bot,$filename) = @tempdata;
        print STDERR "chk3: page = $page, filename=$filename\n";

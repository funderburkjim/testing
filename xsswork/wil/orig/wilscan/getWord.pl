...
# 10-09-2020: Comment out transcoding logic. Only HK supported
use CGI;
...
my $transLit= $cgi->param('transLit');
if (!($transLit)) {
    $transLit="HK";
}
$filter = "HK";
...
if (($keyin) && ($keyin =~ /^[0-9]+$/)) {
    ...
    $key = $keyin;
#    $key = translate_string2HK($transLit,$keyin);
}
my $err = 0;
...
print $info;
#print "getWord.pl: info=$info";
#if ($filter) { # 10-09-2020
...
#    $sql = "select * from `$table` where `key`=\"$key\"";
    ...
    my ($temp,$page);
    my $n = 0;
    # Feb 16, 2011. Logic changed for page 126 = As, as
    # Asa appears on prior page.
#    if (($id,$data1)=$dbhout->fetchrow_array) {
    
    while (($n == 0) && (($id,$data1)=$dbhout->fetchrow_array)) {
	if ($id eq $key) {
...
#   x = <p>98161</p><d>0-1-108-108-108</d>
    ...
    my ($page,$par,$nh,$nl,$l1,$l2,$lt,$tempdata);
    $page = $1;
    $par = $2;
    $nh = $3;
    $nl = $4;

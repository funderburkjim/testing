...
* TODO Bezout Theorem in N.  
** b = a*q + r (0<r<a) implies gcd(b,a) = gcd(a,r)
  Proof: Assume 1<=a and 1<=b. 
  comdivs(b,a) = comdivs(a,r). 
  gcd(b,a) = Nmax(combdivs(b,a)) = Nmax(comdivs(a,r)) = gcd(a,r)
  
** Bezout theorem 
  If 1 <= a and 1<=b and gcd(a,b) == 1, THEN E[x,E[y, 1<=x , 0<=y, a*x = b*y + 1]]
  Proof:
    0 < b <= a
    a = b*q + r, 0<=r<b.
   Case r == 0: 
    a = b*q. Thus, 1 = gcd(a,b) = b;  a*1 = q.
    q = a, 1<=q.  Set x = 1, y = q-1:  a*x = a; b*y + 1 = b*(q-1)+1 = b*q. 
   Case r != 0. 
   0 < r < b
   1 = gcd(a,b) = gcd(b,r),  
   By induction on b, E[u,E[v r*u = b*v + 1]].
   a*u = (b*u)*q + r*u = b*u*q + b*v + 1 = b*(u*q+v) + 1
--------------
   eqn(a,b,x,y) : IF 0 < b <= a and gcd(a,b) == 1 THEN
     E[x,E[y, 1<=x , 0<=y, a*x = b*y + 1]]
   eqn(a) :  0 < a => A(b, eqn(a,b)
  (1) A(a, eqn(a))
  (2) Case: 0 < a, b == a.
     Then 1 = gcd(a,b) = b.  And a = 1
     Take x = 1, y = 0.  Then
     a*x = 1*1 = 1
     b*y + 1 = 1*0 + 1 = 1
     So, a*x = b*y + 1
  Inductive assumption: A(c, c<a implies eqn(c))
  Inductive step:  eqn(a). Choose b, 0<b<=a and gcd(a,b) == 1.
  The case b == a is done by (2)
  Case  b < a.
   Divide a by b: a = b*q + r, 0<=r<b. 
   Case r = 0.  Thus a = b*q.  Thus 1 = gcd(a,b) = b. 
      Also, if q == 0, then a = r, so a = 0. Contrary to assumption 0<b<=a.
      Thus 1<=q, so q-1 is in N .  Take x = 1, y = q-1.
      a*x = a.
      b*y + 1 = 1*(q-1) + 1 = 1*q = b*q = a
      so a*x = b*y + 1
   Case r!= 0.
    Now gcd(b,r) = gcd(a,b) = 1.  
    We may apply the inductive assumption with 'b' for 'c'
     Thus eqn(b,r,u,v) holds:
      IF 0 < r <= b and gcd(b,r) == 1 THEN
        E[u,E[v, 1<=u , 0<=v, b*u = r*v + 1]]
    0<r<b. and gcd(b,r) = 1.  So we may choose u, v
       1<=u , 0<=v, b*u = r*v + 1
    
  eqn(a+1,b): IF 0 < b <= a+1 and gcd(a+1,b) == 1 THEN
     E[x,E[y, 1<=x , 0<=y, a*x = b*y + 1]]
  Case a = 1.  0<b<=1 implies b == 1, so b == a. So (2) applies for answer
...
* Proof that relprimes(p) has a primitive root if p is a prime.
** If gcd(a,n) = 1, then there is 1<=u with a**u == 1 (mod n).
  Proof: From Euclid lemma corollary, if gcd(a,n) = 1 and gcd(b,n) = 1,
   then gcd(a*b,n) = 1.
...
** If gcd(a,n) = 1, properties of ord(a,n)
   1 <= ord(a,n) and a**ord(a,n) = 1 (mod n) 
   and A(k, if 1<=k and a**k = 1 (mod n), then ord(a,n) <= k).
** if gcd(a,n) = 1, then ord(a,n) divides NCard(relprimes(n)) = ephi(n)
...
** ordCover(n,d)
  ...
  If ordCover(n,d) is not empty, then d divides NCard(relprimes(n)) = ephi(n).
...
** ordCoverF(n)
  ...
  Define ordCoverSizeF(n) sending d to NCard(ordCover(n,d)).
  Then divisorsum(ordCoverSizeF(n), ephi(n)) = 
   Sum(NCard(ordCover(n,d)) for d in divisors(ephi(n)))
** divisorsum(ordCoverSizeF(n), ephi(n))  = ephi(n)
  This is because range(ordCoverF(n)) is a disjoint cover of relprimes(n).
** if n is a prime,
** If n is  a prime and d divides ephi(n), then NCard(ordCover(n,d)) <=  ephi(d)
  Clear if ordCover(n,d) is empty set. (since 0 <= ephi(d))
  Case ordCover(n,d) is not empty. 
  If a is in ordCover(n,d), a is in relprimes(n) and eltOrder(a,n) = d.
  So, in particular, a**d = 1 (mod n).
  Thus a is a root of the polynomial X**d - 1 in the mod-n ring.
  Further, since n is a prime, 
** 
 So ephi(n) = divisorsum(

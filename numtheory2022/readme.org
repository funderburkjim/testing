...
* NdivQ(n,a) and divisors(n)
 NdivQ(n,a) "n divides a" Iff 1<=n and 0<=a and Rem(a,n) = 0.  
 NOTATION n | a.
 if 0<n, then NdivQ(n,0)
 if in(a,N) and in(n,N*),  then Iff(NdivQ(n,a), a = n*Quo(a,n))
 Iff(NdivQ(n,a), And(in(n,N*), in(a,N) , E(q, a = n*q)))
 divisors(n) = {d, 1<=d<=n and NdivQ(d,n)}.  No
 If n in N*, then 
...
* NgcdQ(a,b,d) 
  ...
  Since
  Ngcd(a,b,d) = Solution()  (there is exactly one such d)
...
* d|a implies (d|(a+b) Iff d|b)
  ...
  Thus, b = d*y.
  Then d|b.
* b = a*q + r (0<r<a, 0<=q) implies gcd(b,a) = gcd(a,r)
...
* Bezout Theorem in N using NidealQ
  ...
  Then S is an Nideal, generated by gcd(a,b).
...
** Let D = NMin(K). Then in(c,K) Iff E[u, 1<=u and c = D*u]
** G = gcd(a,b).  We show D = G.
...
Gauss, Disquisitiones Arithmeticae
  ...
  Let t = NMin(S). Since S is not empty, t is in S.
  So  1<=t<p and p|a*t.
*** t != 1
...
*** Thus, r is in S.
*** Thus t <= r. Contradicting r<t.
*** Thus, we conclude S IS empty set.
...
* Corollary 1 to Euclid's Lemma: gcd(a,n)=1, 1<=x, a|x, n|x => a*n|x.
...
** If 1<n, then 1 is the multiplicative identity in 'n', and and 0!=1 in 'n'.
  ...
  f(a,1) = R(a*1) = R(a) = a.
* homomorphism Nring(a,n) to Nring(b,m)
  x, y in a
  h(x) = Rem(x,m), h(y) = Rem(y,m)
  z = prod(x,y,n) = Rem(x*y,n)
  h(z) = Rem(z,m) = Rem(Rem(x*y,n),m)
  w = prod(h(x),h(y),m) = Rem(
  In Nring(a,n), x*!y = Rem(x*y,a)
  In Nring(b,m), h(x)*@h(y)
* Chinese remainder theorem 
  ...
  Statement: Take 1<=m, 1<=n
   Define h: m*n to carprod(m,n) by h(x) = (Rem(x,m),Rem(x,n)) for x in m*n.
   We aim to prove that h is a ring isomorphism WHEN gcd(m,n) = 1.
  We prove that h is a ring homomorphism provided 1<=m and 1<=n;
  and then that h is bijection when, further, gcd(m,n) = 1.
  Some statements require only 1<=m and 1<=n. The 1<m and 1<n statements
  are relevant when discussing mutliplicative identity.

** Lemma a: 1<=m, 1<=k, NDivQ(m,k), u in N => Rem(Rem(u,k),m) = Rem(u,m)
   u = k*Quo(u,k) + Rem(u,k)
   Rem(u,m) = Rem(Rem(k*Quo(u,k),m) + Rem(Rem(u,k),m),m)
   Since m|k, also m|k*Quo(u,k).  Thus, Rem(Rem(k*Quo(u,k),m),m) = 0
   Rem(u,m) = Rem(Rem(Rem(u,k),m),m) = Rem(Rem(u,k),m)
** Lemma b: 1<=m, 1<=n, u in N => Rem(Rem(u,m*n),m) = Rem(u,m)
   From lemma a, since NDivq(m,k*m).
** 1<=m, 1<=n, fcn f from m*n to m by f(x) = Rem(x,m). 
** f preserves addition
  NSum(m*n)(a,b) = Rem(a+b,m*n)
  NSum(m)(a,b) = Rem(a+b,m)
  f(a) = Rem(a,m), f(b) = Rem(b,m)
  lhs = f(NSum(m*n)(a,b)) = f(Rem(a+b,m*n)) = Rem(Rem(a+b,m*n),m)
  rhs = NSum(m)(f(a),f(b)) = Rem(f(a)+f(b),m)
   = Rem(Rem(a,m)+Rem(b,m),m) = Rem(a+b,m) property of Rem
  By Lemma b, Rem(Rem(a+b,m*n),m) = Rem(a+b,m).
  Thus, lhs = rhs. 
** f preserves multiplication
  NProd(m*n)(a,b) = Rem(a*b,m*n)
  NProd(m)(a,b) = Rem(a*b,m)
  f(a) = Rem(a,m), f(b) = Rem(b,m)
  lhs = f(NProd(m*n)(a,b)) = f(Rem(a*b,m*n)) = Rem(Rem(a*b,m*n),m)
  rhs = NProd(m)(f(a),f(b)) = Rem(f(a)*f(b),m)
   = Rem(Rem(a,m)*Rem(b,m),m) = Rem(a*b,m) property of Rem
  By Lemma b, Rem(Rem(a*b,m*n),m) = Rem(a*b,m).
  Thus, lhs = rhs.
** 1<=m, 1<=n, fcn g from m*n to n by g(x) = Rem(x,n). 
** Similarly, g preserves addition
  NSum(m*n)(a,b) = Rem(a+b,m*n)
  NSum(m)(a,b) = Rem(a+b,n)
  f(a) = Rem(a,n), f(b) = Rem(b,n)
  lhs = f(NSum(m*n)(a,b)) = f(Rem(a+b,m*n)) = Rem(Rem(a+b,m*n),n)
  rhs = NSum(m)(f(a),f(b)) = Rem(f(a)+f(b),n)
   = Rem(Rem(a,n)+Rem(b,n),n) = Rem(a+b,n) property of Rem
  By Lemma b, Rem(Rem(a+b,m*n),n) = Rem(a+b,n).
  Thus, lhs = rhs. 
** Similarly, g preserves multiplication
  NProd(m*n)(a,b) = Rem(a*b,m*n)
  NProd(m)(a,b) = Rem(a*b,n)
  f(a) = Rem(a,n), f(b) = Rem(b,n)
  lhs = f(NProd(m*n)(a,b)) = f(Rem(a*b,m*n)) = Rem(Rem(a*b,m*n),n)
  rhs = NProd(m)(f(a),f(b)) = Rem(f(a)*f(b),n)
   = Rem(Rem(a,n)*Rem(b,n),n) = Rem(a*b,n) property of Rem
  By Lemma b, Rem(Rem(a*b,m*n),n) = Rem(a*b,n).
  Thus, lhs = rhs.
** ring structure on carprod(m,n).
   Sumc((x,y),(z,w)) 
...
** h preserves addition
   lhs = h(NSum(m*n)(x,y)) 
   = h(Rem(x+y,m*n)) 
   = (Rem(Rem(x+y,m*n),m),Rem(Rem(x+y,m*n),n)
   = (Rem(x+y,m),Rem(x+y,n)) by lemmab, twice
   rhs = Sumc(h(x),h(y))
   = Sumc((Rem(x,m),Rem(x,n)) , (Rem(y,m),Rem(y,n)))
   = (Rem(Rem(x,m)+Rem(y,m),m) , Rem(Rem(x,n)+Rem(y,n),n))
   = (Rem(x+y,m),Rem(x+y,n))  by properties of Rem(-,m) and Rem(-,n).
** Similarly, h preserves multiplication
** h preserves multiplicative identity (provided 1<m and 1<n)
   1 in m*n is multiplicative identity (relative to NProd(m*n)).
   h(1) = (Rem(1,m),Rem(1,n)) = (1,1)  since 1<m and 1<n
** h preserves additive identity
   0 in m*n (since 1<=m*n). 0 in m since 1<=m, and 0 in n since 1<=n
   h(0) = (Rem(0,m),Rem(0,n)) = (0,0)
** h is a ring homomorphism.  Summarizes.
** If gcd(m,n) = 1 and x is in m*n and h(x) = (0,0), then x = 0.
...
** If gcd(m,n) = 1 then h is an injection.

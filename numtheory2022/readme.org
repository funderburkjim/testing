...
** 5. BC 5: 0<a, 0<b, 0<c implies gcd(a*c,b*c) = gcd(a,b)*c.
   gcd(a,b) | a and | b.  Thus gcd(a,b)*c | a*c and | b*c.  Thus
   (i) gcd(a,b)*c | gcd(a*c,b*c).
   Conversely, There is 0<x and 0<=y so that a*x = b*y + gcd(a,b).
...
* if p is a prime and d divides p-1, then x^d - 1 = 0 (mod p) has d solutions.
...
** Preliminary: 1<=n implies Y-1 divides Y^n - 1 in R[Y].
 Assume R is a ring with unit. Work in polynomials with coefficients in R.
...
** c) If 1<=k, 1<=q, and 1<=d, then gcd(d*k,d*q) = d*gcd(k,q)
  See above (Bezout Theorem corrolaries, BC 5) for 
   another way to say the proof.
  This proof is:
...
** (7) if G has a primitive root, then NCard(groupPrimRoots(G)) = ephi(NCard(G)).
   ...
   ephi(NCard(G)).
* When product of finite groups is not cyclic
...
** (1) If gcd(a,n) = 1, then there is 1<=u with a**u == 1 (mod n).
   Let G be the group on relprimes(n).
   a is in G.
   G is a finite group.
   Set u = ord(a,G).
   Then 1<=u and a**u = 1
...
** (4) if gcd(a,n) = 1, then ord(a,n) divides NCard(relprimes(n)) = ephi(n)
 The result follows from (6) of 'order of finite group element'.
** (5) eltsOfOrder(d,n)
  1<=n. 
  eltsOfOrder(d,n) = {a in relprimes(n) and ord(a,n) = d}.
  ...
  Example: n = 12. relprimes(12) = {1,5,7,12}, ephi(12) = 4, 4|4, eltsOfOrder(4,12) = empty set
** (6) eltsOfOrderF(n)
...
** (10a) if n is a prime and d | ephi(n), then NCard(roots(X**d -1)) = d.
    ...
    The idea is that there is a polynomial g so that (X**(p-1) - 1) = (X**d - 1)*g.
       (the coefficients are in NRing(n))
    Fermat's little theorem says there are n-1 roots (i.e. {1,...,n-1}) of
    X**(p-1) - 1.  Thus there are d roots of (X**d - 1) (and (n-1)-d roots of g).
...
** (11) If n is a prime and d | ephi(n), then NCard(eltsOfOrder(d,n)) <=  ephi(d)
  Case i: eltsOfOrder(d,n) is empty set, since 0 <= ephi(d)
  Case ii: eltsOfOrder(d,n) is not empty. 
   Let H = roots of X**d - 1 in Nring(n).
   (*) If a is in eltsOfOrder(d,n), then H = gen(a)
    a is in relprimes(n) and ord(a,n) = d.
    H = gen(a), by (10)
   if b is in eltsOfOrder(d,n), then H = gen(b)  (similarly).
   Since eltsOfOrder(d,n) is not empty, we may choose a in eltsOfOrder(d,n).
    Let K = {b, b in relprimes(n) and gen(b) = gen(a)}
    If b is in eltsOfOrder(d,n): 
     H = gen(b), by (*).
    ...
    If b is in K:
     ...
     ord(b,n) = NCard(gen(b)), by 5a1 of 'order of finite group element' above.
       ...
       = d, since a is in eltsOfOrder(d,n).
     Thus b is in eltsOfOrder(d,n).
   Thus, eltsOfOrder(d,n) = K
   NCard(eltsOfOrder(d,n)) = NCard(K)
    = ephi(ord(a,n)),  By section (5d) of section 'order of finite group element' above
    = ephi(d)
   So, in particular, NCard(eltsOfOrder(d,n)) <=  ephi(d).
   Case i and ii together show that always NCard(eltsOfOrder(d,n)) <=  ephi(d).
   QED.
...
** (13) if n is a prime, then divisorsum(ephi,n-1) = n-1.
  Since 1<n.
** (14) if X is finite set, f:X->R* and g:X->R* and sum(f) = sum(g) and A(i,f(i)<=g(i)), then f=g
   i.e., A(i, f(i) = g(i)).

   Here, R* is the set of non-negative real numbers.
   sum(f) = f(X0) + ... f(Xm) where X0,...,XM is some enumeration of X.

   consider h = g-f. Then g:X->R and sum(h) = sum(g)-sum(f) = 0.
   And also A(i,0<=h(i)).  
   Suppose k in X and 0 < h(k)
...
** (15) if n is a prime and d divides n-1, then NCard(eltsOfOrder(d,n)) = ephi(d).
  ...
  Let sum(f) = divisorsum(f,m) for a function defined on D.
  m = ephi(n) = n-1 since n is prime.
  Let f and g be the two functions from D to N defined by
  f(d) = NCard(eltsOfOrder(d,n)) and g(d) = ephi(d).
  By (9), sum(f,D) = n-1 
  By (12), sum(g,D) = n-1
...
** (16) if n is a prime and d divides n-1, then roots(X^d - 1) is cyclic.
 Proof: By (15), NCard(eltsOfOrder(d,n)) = ephi(d). 
 ephi(d) = n-1.
 Since 0 < ephi(d), we conclude eltsOfOrder(d,n) is not empty.
 Thus, there is a in eltsOfOrder(d,n).
...
** (17a) If n is a prime, then there is a primitive root x for n (i.e., primRootQ(x,n).
   From (17)
** (18a) if primRootQ(x,n), then primRoots(n) = {x**k (mod n), for k in relprimes(n))
  by 5d of the group result
** (18b) if primRootQ(x,n), then NCard(primRoots(n)) = NCard(
...
* CODE: related to all primitive roots 
...
* If 3<=k, then relprimes(2**k) is NOT cyclic.
 if 1<=k, then  relprimes(2**k) = {2*i + 1, with 0 <= i < 2**(k-1)}
 So ephi(2**k) = 2**(k-1)
...
** (1) is proved by induction on k.
  Assume x is odd. (1) is trivial if x = 1. 
  So assume further 1<x
  ...
  The k+1 equation is
   ...
   From (a), there is 1<=m with x**(2**(k-2)) = (2**k)*m + 1  (1<=m since 1<x)
...
** Second proof of (1) 
   ...
   n = 1
    2**n = 2**1 = 2. 
    a**2 = 4*(q**q) + 4*q + 1 = 4*(q*(q+1)) + 1
     Now q*(q+1) is even so q*(q+1) = 2*j and
    ...
    lhs = (a**(2**n))**2 = (1 + j*(2**(n+2)))**2 =
        = (j**2)*[(2**(n+2))**2] + 2*j*(2**(n+2))) + 1 = 
         ...
         (2**(n+2))**2 = 2**((n+3)+(n+1)) = (2**(n+1))*C
          A = (j**2)*(2**(n+1))*C  = v*C, where v = (j**2)*(2**(n+1))
    lhs = (j + v)*C + 1 = u*C + 1, where u = j + v.
...
* if m > 2 and n > 2, and gcd(m,n) = 1, then relprimes(m*n) has no primitive element.
  ...
  2 | ephi(m), since 3<=m
  2 | ephi(n), since 3<=n
  2 <= gcd(NCard(G),NCard(H)) , 
  gcd(NCard(G),NCard(H)) != 1.
  carprod(G,H) is not cyclic (theorem above)
...
** Binomial theorem:
  ...
  if 3<=n, then (a+b)**n = a**n + n*(a**(n-1))*(b**1) + C(n,2)*(a**(n-2))*(b**2) + (mod b**3)
...
** (1a) if p is prime and 1<=k and PR(r,p**k) then ephi(p**k) | ord(r,p**(k+1)) | ephi(p**(k+1))
   ...
   ord(r,pk)  
** (1b) if p is a prime and PR(r,p), then (p-1) | ord(r,p2) | p*(p-1)
   Proof:
   1<=r<p. set d = ord(r,p) = p-1.  
   r**d = 1 (mod p) and 1<=k<p-1 implies r**k != 1 (mod p)
   gcd(r,p2) = 1. 
   1<=r<p2, since p < p2.
   ...
   Let e = ord(r,p2). 
    1 <= e
    r**e = 1 (p2) 
    e | p*(p-1), the size of relprimes(p2), and 
    r**e = 1 (p), since p | p2.
    d | e  [Cf. (3b) of 'order of finite group element']
     (p2 divides r*e - 1, p divides p2, so p divides r**e - 1.)
...
** (3) if p is a prime and PR(r,p) and r**(p-1) != 1 (mod p2), then PR(r,p2).
   1<=r<p so 1<=r<p2.  gcd(r,p) = 1.  If d divides r and p2, then d divides r and p.
   So gcd(r,p2) = 1.
   Let e = ord(r,p2). So e is smallest positive integer such that r**e = 1 (mod p2)
   By (1), (p-1) | e | p*(p-1).  
   By (2), either e = (p-1) or e = p*(p-1), since p is prime..
   If e = (p-1), then r**(p-1) = 1 (mod p2), contrary to assumption.
   Thus, e = p*(p-1), which is the size of relprimes(p2). 
   Thus, r is a primitive root for p2
** (4) if p is a prime and 1<=r<p, then r+p is in relprimes(p2)
...
** (5) if p an odd prime, then (a+b)**(p-1) = (a**(p-1)) + (p-1)*(a**(p-2))*b (mod p2)
...
** (7) if p is odd prime and PR(r,p) and NOT PR(r,p2), then PR(r+p,p2)
   ...
   assert: (p-1) | f
    ...
    And (r+p)**f = 1 (mod p2), so also (r+p)**f = 1 (mod p) (since p divides p2).
    Thus r**f = (r+p)**f  = 1 (mod p).
    Thus ord(r,p) | f. Thus (p-1) | f.
   PR(r+p,p2):
    By (2), either either f = (p-1) or f =  p*(p-1).
    Now, (r+p)**f = 1 (mod p2) (defn. of ord). 
    Combine this with (*) to get f != (p-1).
    Thus f = p*(p-1).  Thus (r+p) is primitive element for p2.
   QED
...
** (10a) if p is an odd prime and PR(r,p**2), then PR(r,p**3).
   Let p2 = p**2, p3 = p**3, p1 = p**1 = p.
   By assumption, r is in relprimes(p2).
   r is in relprimes(p3), since relprimes(p2) is a subset of relprimes(p3).
   We assume that ord(r,p2) = ephi(p2).
   We want to show that ord(r,p3) = ephi(p3).
   ephi(p1) = p-1
   ephi(p2) = p*(p-1)  = ephi(p1)*p
   ephi(p3) = p2*(p-1) = ephi(p2)*p
   ephi(p1) | p2,
   ephi(p1) < ephi(p2), since 2 < p by assumption
   r**ephi(p1) != 1 (mod p2), since ord(r,p2) = ephi(p2).
   Suppose we can show r**ephi(p2) != 1 (mod p3):
...
** (10) if p is an odd prime and PR(r,p2), then A(k, 2<=k implies PR(r,p**k))
   ...
   Since relprimes(p**j) is a subset of relprimes(p**(j+1)) for all j, 
   then r is in relprimes(p**j) for all j with 2<=j.
...
*** Inductive hypothesis: PR(r,pk). To show PR(r,p**(k+1)).
   r in relprimes(pk) and  ord(r,pk) = ephi(pk) = (p**(k-1))*(p-1).
     So r**ephi(pk) = 1 (mod pk), 
     A(j, 0 < j < ephi(pk) implies r**j != 1 (mod pk))
   r**((p**(k-2))*(p-1)) != 1 (mod pk), since ((p**(k-2))*(p-1)) < ephi(pk).

   To prove for k+1:  r**((p**((k+1)-2))*(p-1)) != 1 (mod pk1).
     i.e., r**((p**(k-1))*(p-1)) != 1 (mod pk1)
   Now ((p**(k-2))*(p-1)) = ephi(p**(k-1)),
   Since gcd(r,p2) = 1, then also gcd(r,p) = 1, so also gcd(r,p**(k-1)) = 1.
 ...
 Case k = 2
  Assume: r**((p**(2-2))*(p-1)) != 1 (mod p2).  or r**(p-1) != 1 (mod p2)
...
** (11) if p is an odd prime and 1<=k, then there is s so that PR(s,p**k).
